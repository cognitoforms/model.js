...
export abstract class Format<T> {
	...
	constructor(model: Model, specifier: string, description: string = null, nullString: string = null, undefinedString: string = undefined) {
		if (process.env.NODE_ENV === "development") {
			if (!model || !(model instanceof Model)) throw new Error("Model must be provided.");
			if (!specifier || typeof specifier !== "string") throw new Error("Format specifier string must be provided.");
		}
	...
	static create<T>(model: Model, options: CustomFormatOptions<T>): Format<T> {
	...
	static hasTokens(template: string): boolean {
		return getFormatTemplateParser().test(template);
	}
}
...
export class CustomFormat<T> extends Format<T> {
	...
	constructor(model: Model, options: CustomFormatOptions<T>) {
		super(model, options.specifier, options.description, options.nullString, options.undefinedString);
...
export class ModelFormat<T extends Entity> extends Format<T> {
	...
	compile(): void {
		if (!this.tokens) {
			...
			var index = 0;
			const formatTemplateParser = getFormatTemplateParser();
...
export function createFormat<T>(model: Model, type: any, format: string): Format<T> {
	if (type === Date) {
		// Add support for g and G that are not natively supported by the format and parse methods
		if (format === "g")
			format = expandDateFormat(model.$culture.dateTimeFormat, "d") + " " + expandDateFormat(model.$culture.dateTimeFormat, "t");
		else if (format === "G")
			format = expandDateFormat(model.$culture.dateTimeFormat, "d") + " " + expandDateFormat(model.$culture.dateTimeFormat, "T");

		return Format.create<Date>(model, {
			specifier: format,
			description: "",
			paths: [],
			convert: function (value: Date): string {
				return formatDate(value, format, model.$culture);
			},
			convertBack: function (str: string): Date | FormatError {
				...
				if (format === "t") {
					var timeFormat = expandDateFormat(model.$culture.dateTimeFormat, "d") + " " + expandDateFormat(model.$culture.dateTimeFormat, "t");
					var startDate = formatDate(new Date(1970, 0, 1), "d", model.$culture);
					date = parseDate(startDate + " " + str, model.$culture, [timeFormat]);
				}
				else
					date = parseDate(str, model.$culture, [format]);
		...
		return Format.create<number>(model, {
			...
			convert: function (val: number): string {
				...
				return formatNumber(val, format, model.$culture);
			},
			convertBack: function (str: string): number | FormatError {
				var result = parseNumber(str, getNumberStyle(format), model.$culture);
